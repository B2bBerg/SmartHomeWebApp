...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    -- FDW Mapping für den Admin/Initialisierungs-User auf die dbStatic
    ...
    -- 3. Lese- & Schreibrechte auf Tabellen, Hypertables & Continuous Aggregates
    GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO ${APP_DB_USER};
    ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO ${APP_DB_USER};

    -- 4. User-Mapping für FDW: Erlaubt dem app_user Anfragen an die dbStatic durchzuführen
