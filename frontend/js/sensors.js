...
const SensorManager = {
    ...
    async loadData() {
        try {
            const [sensors, devices] = await Promise.all([window.API.getSensors(), window.API.getDevices()]);
    ...
    async showAddModal() {
        ...
        modal.innerHTML = `
            <div class="table-modal-box">
                <h3>Neuen Sensor erfassen</h3>
                <p>Suchen Sie das Master-Gerät (Hardware) für den neuen Datenpunkt:</p>
                <div class="settings-group">
                    <label>Name *</label>
                    <input type="text" id="modal-name" placeholder="Eigener Name (z.B. Temperatur Wohnzimmer)">
                </div>
                <div class="settings-group">
    ...
    async showEditModal(row) {
        ...
        btnConfirm.onclick = async () => {
            ...
            try { await window.API.updateSensor(row.id, row); this.table._render(); modal.remove(); } 
    ...
    async deleteSensor(rowId) {
        try {
            await window.API.deleteSensor(rowId);
